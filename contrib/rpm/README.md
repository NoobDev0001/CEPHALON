...
The RPM spec file provided here is for Cephalon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://cephalon.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Cephalon git repository](https://github.com/CephalonChain/Cephalon) but are not part of
...
Currently only `Source10` is used, for the example `cephalon.conf` file.
...
in the cephalon git repository. At present only an SVG version of the Cephalon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the cephalon
...
needed when 0.12.1 is released, a proper fix is already in the Cephalon git
master branch.
...
idea to build Cephalon Core against a static version of that library compiled
...
The `cephalon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `cephalon-server` package. At this point, most RPM based
...
The `cephalon-server` package should have SELinux support. How to properly do
...
Cephalon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/cephalon/). He has been
packaging Cephalon for Fedora at least since 2012.
...
4. I renamed the `cephalon` package that contains the Qt GUI to `cephalon-core` as
that appears to be how the general population refers to it, in contrast to
`cephalon-xt` or `cephalon-classic`. I wanted to make sure the general population
