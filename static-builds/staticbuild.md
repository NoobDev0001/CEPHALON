...
sudo find ${DISTNAME}/bin -type f -executable -exec ../cephaloncoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../cephaloncoin/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo cephalon-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find cephalon-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/cephaloncoin
...
cp -f cephalon-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
sudo rm -rf win64
cp -rf cephaloncoin/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir unsigned
mv ~/cephaloncoin/cephalon-*setup-unsigned.exe unsigned/
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/cephalon-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer cephalon-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer cephalon-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf cephaloncoin/contrib/windeploy ~/sign/win32
...
#transfer cephalon-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/cephaloncoin/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/CephalonChain/Cephalon
mkdir ~/cephaloncoin/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder cephaloncoin/depends/SDKs
cd cephaloncoin/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/cephaloncoin
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Cephalon-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/cephalon-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer cephalon-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
