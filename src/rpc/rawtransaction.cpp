...
{
    // Call into TxToUniv() in cephalon-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in cephalon-common, so we query them here and push the
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    ...
                    if (spentInfo.addressType == 1) {
                        in.pushKV("address", CCephalonAddress(CKeyID(spentInfo.addressHash)).ToString());
                    } else if (spentInfo.addressType == 2) {
                        in.pushKV("address", CCephalonAddress(CScriptID(spentInfo.addressHash)).ToString());
                    }
                }
                newVin.push_back(in);
            }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"address\":                          (string, required) The destination cephalon address.\n"
            ...
            // TODO if we decide to remove the consensus check from issue 675 https://github.com/CephalonChain/Cephalon/issues/675
...
// TODO, if we decide to remove the consensus check https://github.com/CephalonChain/Cephalon/issues/675, remove or add the code (requires consensus change)
...
//                    }
                ...
                else if (assetKey_ == "reissue")
                {
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, owner_change_address is not a valid Cephalon address");
                    ...
                    if (fHasOwnerChange && !IsValidDestinationString(owner_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER,
                                           "Invalid parameter, owner_change_address is not a valid Cephalon address");
                    ...
                    if (fHasRootChange && !IsValidDestinationString(root_change_address.get_str()))
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, root_change_address is not a valid Cephalon address");
                    ...
                    for (int i = 0; i < (int)addresses.size(); i++) {
                        if (!IsValidDestinationString(addresses[i].get_str()))
                            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, supplied address is not a valid Cephalon address");
                    }
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) cephalon address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
