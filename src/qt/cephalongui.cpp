...
const std::string CephalonGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString CephalonGUI::DEFAULT_WALLET = "~Default";

CephalonGUI::CephalonGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void CephalonGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a cephalon address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and cephalon: URIs)"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your cephalon addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified cephalon addresses"));
    ...
    openAction->setStatusTip(tr("Open a Cephalon: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible cephalon command-line options").arg(tr(PACKAGE_NAME)));
...
void CephalonGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void CephalonGUI::createToolBars()
{
    if(walletFrame)
    {
        /** CEPHALON START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/cephaloncointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* cephalonLabelLayout = new QVBoxLayout(toolbarWidget);
        cephalonLabelLayout->addWidget(label);
        cephalonLabelLayout->addWidget(toolbar);
        cephalonLabelLayout->setDirection(QBoxLayout::TopToBottom);
        cephalonLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Cephalon Market Price"));
        ...
        // Get the latest Cephalon release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void CephalonGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool CephalonGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool CephalonGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void CephalonGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void CephalonGUI::setWalletActionsEnabled(bool enabled)
{
...
void CephalonGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void CephalonGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void CephalonGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void CephalonGUI::optionsClicked()
{
...
void CephalonGUI::showDebugWindow()
{
...
void CephalonGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void CephalonGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void CephalonGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void CephalonGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void CephalonGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void CephalonGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void CephalonGUI::openClicked()
{
...
void CephalonGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void CephalonGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void CephalonGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void CephalonGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void CephalonGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void CephalonGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CEPHALON START */
void CephalonGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void CephalonGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void CephalonGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void CephalonGUI::gotoRestrictedAssetsPage()
{
...
void CephalonGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to cephalon network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void CephalonGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void CephalonGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void CephalonGUI::updateHeadersSyncProgressLabel()
{
...
void CephalonGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void CephalonGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Cephalon"); // default title
...
void CephalonGUI::changeEvent(QEvent *e)
{
...
void CephalonGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void CephalonGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void CephalonGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "NEOX")
        msg += tr("Amount: %1\n").arg(CephalonUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(CephalonUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void CephalonGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void CephalonGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool CephalonGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool CephalonGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void CephalonGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void CephalonGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void CephalonGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void CephalonGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void CephalonGUI::detectShutdown()
{
...
void CephalonGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void CephalonGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void CephalonGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(CephalonGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(CephalonGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void CephalonGUI::subscribeToCoreSignals()
{
...
void CephalonGUI::unsubscribeFromCoreSignals()
{
...
void CephalonGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void CephalonGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<CephalonUnits::Unit> units = CephalonUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const CephalonUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(CephalonUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(CephalonUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (CephalonUnits::Unit u : CephalonUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(CephalonUnits::name(u)), this);
...
{
    setText(CephalonUnits::name(newUnits));
}
...
void CephalonGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/CephalonChain/Cephalon/releases"));
