...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + CephalonUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid cephalon Destination Address"));
            return;
        }
    }
