Cephalon Core version *0.15.0* is now available from:

  <https://cephalon.net/bin/cephalon-core-0.15.0/>
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/Bitcoin/bitcoin/pull/10707)):
    ...
    - If Cephalon Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
- A new `estimaterawfee` RPC is added to provide raw fee data. External clients can query and use this data in their own fee estimation logic.

Multi-wallet support
--------------------

Cephalon Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/Bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/Bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Cephalon, either on the command line or in the Cephalon config file.

**In Cephalon-Qt, only the first wallet will be displayed and accessible for creating and signing transactions.** GUI selectable multiple wallets will be supported in a future version. However, even in 0.15 other loaded wallets will remain synchronized to the node's current tip in the background. This can be useful if running a pruned node, since loading a wallet where the most recent sync is beyond the pruned height results in having to download and revalidate the whole blockchain. Continuing to synchronize all wallets in the background avoids this problem.

Cephalon Core 0.15.0 contains the following changes to the RPC interface and `cephalon-cli` for multi-wallet:

* When running Cephalon Core with a single wallet, there are **no** changes to the RPC interface or `cephalon-cli`. All RPC calls and `cephalon-cli` commands continue to work as before.
* When running Cephalon Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `cephalon-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Cephalon Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:8766/wallet/wallet1.dat/`. `cephalon-cli` commands should be run with a `-rpcwallet` option, for example `cephalon-cli -rpcwallet=wallet1.dat getbalance`.
...
Cephalon Core has supported creating opt-in replace-by-fee (RBF) transactions
...
In previous versions of Cephalon Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Cephalon Core 0.15 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/Bitcoin/bitcoin/pull/9602)). This has the following implications:
...
- `uptime` returns the total runtime of the `cephalond` server since its last start (See [PR 10400](https://github.com/Bitcoin/bitcoin/pull/10400)).

Low-level RPC changes
---------------------

- When using Cephalon Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- Error codes have been updated to be more accurate for the following error cases (See [PR 9853](https://github.com/Bitcoin/bitcoin/pull/9853)):
  ...
  - `removeprunedfunds` now returns RPC_WALLET_ERROR if `cephalond` is unable to remove
  the transaction. Previously returned RPC_INTERNAL_ERROR.
  - `removeprunedfunds` now returns RPC_INVALID_PARAMETER if the transaction does not
  exist in the wallet. Previously returned RPC_INTERNAL_ERROR.
  - `fundrawtransaction` now returns RPC_INVALID_ADDRESS_OR_KEY if an invalid change
  address is provided. Previously returned RPC_INVALID_PARAMETER.
  - `fundrawtransaction` now returns RPC_WALLET_ERROR if `cephalond` is unable to create
...
- #9672 `46311e7` Opt-into-RBF for RPC & cephalon-tx (luke-jr)
...
- #10400 `1680ee0` [RPC] Add an uptime command that displays the amount of time (in seconds) cephalond has been running (rvelhote)
- #10683 `d81bec7` rpc: Move the `generate` RPC call to rpcwallet (laanwj)
- #10710 `30bc0f6` REST/RPC example update (Mirobit)
- #10747 `9edda0c` [rpc] fix verbose argument for getblock in cephalon-cli (jnewbery)
...
- #10228 `27faa6c` build: regenerate cephalon-config.h as necessary (theuni)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple cephalonds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run cephalon_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call CephalonTestFramework start/stop node methods (jnewbery)
...
- #10636 `4bc853b` [qa] util: Check return code after closing cephalond proc (MarcoFalke)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into CephalonTestFramework (jnewbery)
...
- #10090 `8e4f7e7` Update cephalon.conf with example for pruning (coinables)
...
- #10447 `f259263` Make cephalond invalid argument error message specific (laanwj)
- #10495 `6a38b79` contrib: Update location of seeds.txt (laanwj)
- #10469 `b6b150b` Fixing typo in rpcdump.cpp help message (keystrike)
- #10451 `27b9931` contrib/init/cephalond.openrcconf: Don't disable wallet by default (luke-jr)
...
- #10812 `c5904e8` [utils] Allow cephalon-cli's -rpcconnect option to be used with square brackets (jnewbery)
...
- #9999 `a328904` [LevelDB] Plug leveldb logs to cephalon logs (NicolasDorier)
...
- #10958 `659c096` Update to latest Cephalon patches for LevelDB (sipa)
