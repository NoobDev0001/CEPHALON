...
A command-line option (-noseed) can be passed when starting cephalond and/or cephalon-qt for the first time to create a wallet using the original key system that existed in Bitcoin and Cephalon on Jan 3, 2019.

When using the 12-word seed which will be the new default, the address derivation will change to be compatible with the BIP32/BIP39/BIP44 standards.  Since this is already implemented in the Cephalon mobile wallet, the 12-words will be cross-compatible.  The derivation path is m/44'/1668'/0'/0 for the first non-change address.
...
When creating the wallet.dat for the first time, the default should be generating 128 bits of entropy, and storing either the words, or the 132 bits (which includes a 4 bit checkum).  If, however, the -noseed option is set via command-line or cephalon.conf, then the orginal method of creating a master key should be used, and the original path derivation should be used.
...
### Compatibility
Other wallets like Jaxx and Coinomi use a 12-word seed.  For wallets that use BIP39/BIP32/BIP44 and the correct coinid of 1668 for Cephalon, the 12-words should be compatible with external wallets.

Because the amount of NEOX in the asset UTXO is 0, and because the Cephalon transaction will be invalid if the asset outputs don't match the asset inputs, this prevents external wallets from being able to lose assets even though the external wallets are completely unaware of assets.
